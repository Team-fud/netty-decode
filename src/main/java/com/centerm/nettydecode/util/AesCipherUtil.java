...
import com.centerm.nettydecode.util.common.Base64ConvertUtil;
import com.centerm.nettydecode.util.common.HexConvertUtil;
...
public class AesCipherUtil {
    ...
    public static String enCrypto(String str) {
        try {
            ...
            throw new CustomException("getInstance()方法异常:" + e.getMessage());
        } catch (UnsupportedEncodingException e) {
            logger.error("Base64加密异常:{}", e.getMessage());
            throw new CustomException("Base64加密异常:" + e.getMessage());
        } catch (InvalidKeyException e) {
            logger.error("初始化Cipher对象异常:{}", e.getMessage());
            throw new CustomException("初始化Cipher对象异常:" + e.getMessage());
        } catch (IllegalBlockSizeException | BadPaddingException e) {
            logger.error("加密异常，密钥有误:{}", e.getMessage());
            throw new CustomException("加密异常，密钥有误:" + e.getMessage());
        }
    }
    ...
    public static String deCrypto(String str) {
        try {
            ...
            throw new CustomException("Base64解密异常:" + e.getMessage());
        } catch (InvalidKeyException e) {
            logger.error("初始化Cipher对象异常:{}", e.getMessage());
            throw new CustomException("初始化Cipher对象异常:" + e.getMessage());
        } catch (IllegalBlockSizeException | BadPaddingException e) {
            logger.error("解密异常，密钥有误:{}", e.getMessage());
            throw new CustomException("解密异常，密钥有误:" + e.getMessage());
        }
    }
}
